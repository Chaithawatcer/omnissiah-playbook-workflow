...
**Splunk — ตรวจ RDP Brute Force (Event ID 4625 Type 10, จำนวนมากจาก IP เดียว):**
```spl
index=windows_security EventCode=4625 Logon_Type=10
| stats count by src_ip, Account_Name, ComputerName
| where count > 20
| sort -count
| eval alert="RDP Brute Force Suspected"
```

**Splunk — ตรวจ RDP Successful Login จาก unusual IP (Event ID 4624 Type 10):**
```spl
index=windows_security EventCode=4624 Logon_Type=10
| lookup geoip src_ip OUTPUT country_name
| where country_name NOT IN ("Thailand","Singapore")
| stats count by src_ip, Account_Name, ComputerName, country_name
| sort -count
```

**Splunk — ตรวจ RDP login สำเร็จหลังมี failure หลายครั้ง (brute force สำเร็จ):**
```spl
index=windows_security EventCode IN (4625, 4624) Logon_Type=10
| sort _time
| streamstats count(eval(EventCode=4625)) as fail_count by src_ip, Account_Name
| where EventCode=4624 AND fail_count > 5
| table _time, src_ip, Account_Name, ComputerName, fail_count
```

**Splunk — ตรวจ Event ID 1149 (RDP Authentication):**
```spl
index=windows EventSource="Microsoft-Windows-TerminalServices-RemoteConnectionManager" EventCode=1149
| stats count by src_ip, User, host
| where count > 10
| sort -count
```

**CLI — PowerShell ตรวจ Event 4625 Logon Type 10 ล่าสุด:**
```powershell
...
**CLI — ตรวจ event TerminalServices:**
```powershell
...
### Sub: scope_analysis
- ตรวจสอบว่า **RDP attack เป็น targeted** (account เฉพาะ) หรือ **generic scan** (ลอง common username)
- ยืนยันว่ามี **successful login** เกิดขึ้นหรือไม่ (Event 4624 Type 10)
- ถ้า login สำเร็จ ตรวจสอบทันที: attacker **ทำอะไรบ้างหลัง login** (Sysmon Event 1, process ที่รัน)
- ระบุว่า **server ที่ถูก attack** มี data สำคัญอะไร หรือเป็น stepping stone ไปยัง network ภายใน
- ตรวจสอบว่า **RDP exposed โดยตรงจาก internet** หรือผ่าน VPN/Jump host
- ประเมิน **account ที่ถูก lockout** และผลกระทบต่อ business operation
